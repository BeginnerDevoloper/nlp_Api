...
const openai = new openai(process.env[openai]);

app.post('/openai',validateApiKey,async(req,res)=>{
    ...
    try {
       ...
       await mongoose.model('User').updateOne({id:userId},{$set:{prompt:prompt,completion:result.data}})
...
app.get('/openai',validateApiKey,async(req,res)=>{
    const jwt=req.cookies.jwt;
    const decoded=jwt.decode();
    const userId=decoded.id;
    try {
        ...
        const result=await mongoose.model('User').findOne({id:userId},{prompt:1,completion:1});
